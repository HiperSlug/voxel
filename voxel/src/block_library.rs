    ...
    fn load(
        ...
        async move {
            ...
            let raw = match settings {
                Json => serde_json::de::from_slice::<BlockLibraryRaw>(&bytes)
                    .context("Failed to deserialize BlockLibraryRaw from JSON")?,
                Ron => ron::de::from_bytes::<BlockLibraryRaw>(&bytes)
...
pub fn load_block_library(mut commands: Commands, asset_server: Res<AssetServer>) {
