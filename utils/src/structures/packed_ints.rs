...
impl<I: FullInt> PackedInts<I> {
	...
	pub fn max(&self) -> I { self.max_and_mask }
}
...
mod test {
	...
	fn bits_per() {
		...
		PackedInts::<usize>::new(1, 0).unwrap().decrement_bits_per().unwrap_err();

		PackedInts::<usize>::new(PackedInts::<usize>::MAX_BITS_PER, 0).unwrap().increment_bits_per().unwrap_err();
